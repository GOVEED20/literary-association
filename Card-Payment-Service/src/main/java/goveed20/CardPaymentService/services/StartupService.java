...
public class StartupService {
    ...
    public void addEntities() {
        if (bankRepository.findByName("UniCredit").isEmpty()) {
            ...
            // merchant
            OnlinePaymentData oyp = OnlinePaymentData.builder().merchantID("123456789").merchantPassword("satipo").build();
            Client merchant = Client.builder().balance(100.00).onlinePaymentData(oyp).PAN("1234-5611-1111-1111").build();
            unicredit.getClients().add(merchant);

            // customer unicredit
            Card card1 = Card.builder().securityCode("123").cardHolderName("Satoshi").expiryDate("07/2024").build();
            Client customerUnicredit = Client.builder().balance(50.00).card(card1).PAN("1234-5611-2222-3333").build();
            unicredit.getClients().add(customerUnicredit);

            // customer komercijalna
            Card card2 = Card.builder().securityCode("456").cardHolderName("Vitalik").expiryDate("06/2023").build();
            Client customerKomercijalna = Client.builder().balance(50.00).card(card2).PAN("9876-5411-2222-3333").build();
